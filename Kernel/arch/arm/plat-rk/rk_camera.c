...
static int rk_sensor_io_init(void)
{
	...
	for(i = 0;i < RK_CAM_NUM; i++){
        if (plat_data->gpio_res[i].dev_name == NULL)
            continue;
	printk("rk_sensor_io_init:plat_data->gpio_res[i].dev_name:%s\n",plat_data->gpio_res[i].dev_name);
	//wisky-lxh@20120913,camera compatible
	if (!strcmp(plat_data->gpio_res[i].dev_name,"nc_back"))
		continue;
	if (!strcmp(plat_data->gpio_res[i].dev_name,"nc_front"))
		continue;
	...
	printk("camera_powerdown:%d\n",camera_powerdown);
		...
		//wisky-lxh@20120913,run to here indicate the same gpio has been request,we free gpio resource	
		gpio_free(camera_power);
		ret = gpio_request(camera_power, "camera_power");
              if (ret) {
                printk("%s..%s..camera_power pin(%d) init failed\n",__FUNCTION__,plat_data->gpio_res[i].dev_name,camera_power);
                goto sensor_io_init_erro;
            	}
		//end-wisky-lxh@20120913		
		...
		ret = gpio_request(camera_reset, "camera_reset");
              if (ret) {
                printk("%s..%s..camera_reset pin(%d) init failed\n",__FUNCTION__,plat_data->gpio_res[i].dev_name,camera_reset);
                goto sensor_io_init_erro;
            	}
		//end-wisky-lxh@20120913	
		...
		gpio_set_value(camera_powerdown,GPIO_HIGH); 
            if (ret) {
                for (j=0; j<i; j++) {
		...
		ret = gpio_request(camera_flash, "camera_flash");
              if (ret) {
                printk("%s..%s..camera_flash pin(%d) init failed\n",__FUNCTION__,plat_data->gpio_res[i].dev_name,camera_flash);
                goto sensor_io_init_erro;
            	}
		//end-wisky-lxh@20120913
