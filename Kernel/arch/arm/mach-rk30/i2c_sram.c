...
void __sramfunc rk30_suspend_voltage_set(unsigned int vol)
{
    ...
    sram_i2c_init();  //init i2c device
   	{
	slaveaddr = (0x68 >> 1);            //slave device addr
	slavereg = 0x27;            // reg addr
	data = 0xe;       //set arm 1.0v

	logic_voltage=sram_i2c_read(slaveaddr, 0x23);
	
	arm_voltage = sram_i2c_read(slaveaddr, slavereg);
	sram_i2c_write(slaveaddr, slavereg, data);//0.9v
	sram_i2c_write(slaveaddr, 0x23, data);//0.9v
	}
...
void __sramfunc rk30_suspend_voltage_resume(unsigned int vol)
{
	...
	sram_i2c_write(slaveaddr, 0x23, logic_voltage);
	sram_udelay(20000);
	}
    sram_i2c_deinit();  //deinit i2c device
}
