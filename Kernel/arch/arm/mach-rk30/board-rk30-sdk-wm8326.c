...
//	printk("%s set dcdc3 vcc_ddr=%dmV end\n", __func__, regulator_get_voltage(dcdc));
	...
	#else	
	regulator_set_suspend_voltage(ldo, 0);//2800000
	#endif
	regulator_enable(ldo);
//	printk("%s set ldo7 vcc28_cif=%dmV end\n", __func__, regulator_get_voltage(ldo));
	...
	#else	
	regulator_set_suspend_voltage(ldo, 0);//1800000
	#endif
	regulator_enable(ldo);
...
#if 1
	#if 0//defined(CONFIG_RTL8192CU)	
	regulator_set_voltage(ldo, 0, 0);//3300000
	regulator_set_suspend_voltage(ldo, 0);
	#else
	regulator_set_voltage(ldo, 3300000, 3300000);
	regulator_set_suspend_voltage(ldo, 0);
	#endif
