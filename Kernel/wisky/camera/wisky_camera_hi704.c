...
extern int f_cam_probe_success_flag;
...
{
#if defined(WISKY_CAMERA_REG_F)
	#include WISKY_CAMERA_REG_F
#else
	#include "wisky_camera_hi704_default.h"
#endif
};
...
{0x9c, 0x0b}, //EXP Limit 250.00 fps 
{0x9d, 0xb8}, 
{0x9e, 0x00}, //EXP Unit 
{0x9f, 0xfa}, 
{END_REG, END_REG},
...
static int sensor_write_array(struct i2c_client *client, struct reginfo *regarray)
{
    ...
    while (regarray[i].reg != END_REG)
    {
        ...
        {
            if (cnt-- > 0) {
			    pr_err("%s..write failed current reg:0x%x, Write array again !\n", SENSOR_NAME_STRING(),regarray[i].reg);
				i = 0;
				continue;
            } else {
                pr_err("%s..write array failed!!!\n", SENSOR_NAME_STRING());
                err = -EPERM;
				goto sensor_write_array_end;
            }
        } else {
			...
			if (valchk != regarray[i].val)
				pr_err("%s Reg:0x%x write(0x%x, 0x%x) fail\n",SENSOR_NAME_STRING(), regarray[i].reg, regarray[i].val, valchk);
		#endif
        }
        i++;
    }
...
static int sensor_readchk_array(struct i2c_client *client, struct reginfo *regarray)
{
    ...
    while (regarray[i].reg != 0)
    {
		sensor_read(client, regarray[i].reg, &valchk);
		if (valchk != regarray[i].val)
			pr_err("%s Reg:0x%x read(0x%x, 0x%x) error\n",SENSOR_NAME_STRING(), regarray[i].reg, regarray[i].val, valchk);
...
static int sensor_ioctrl(struct soc_camera_device *icd,enum rk29sensor_power_cmd cmd, int on)
{
	...
	switch (cmd)
	{
		...
		default:
		{
			pr_err("%s %s cmd(0x%x) is unknown!",SENSOR_NAME_STRING(),__FUNCTION__,cmd);
			break;
		}
	}
sensor_power_end:
...
static enum hrtimer_restart flash_off_func(struct hrtimer *timer){
	...
	WPRINTK("%s %s !!!!!!",SENSOR_NAME_STRING(),__FUNCTION__);
    return 0;
...
static int sensor_init(struct v4l2_subdev *sd, u32 val)
{
    ...
    if (ret != 0)
    {
        pr_err("%s soft reset sensor failed\n",SENSOR_NAME_STRING());
        ret = -ENODEV;
		goto sensor_INIT_ERR;
    }
    ...
    if (ret != 0) {
        pr_err("read chip id high byte failed\n");
        ret = -ENODEV;
        goto sensor_INIT_ERR;
    }
    ...
    if (value == SENSOR_ID) {
        ...
        pr_err("error: %s mismatched   pid = 0x%x\n", SENSOR_NAME_STRING(), value);
        ret = -ENODEV;
        goto sensor_INIT_ERR;
    }
#endif
    ...
    if (!fmt) {
        pr_err("error: %s initial array colour fmts is not support!!",SENSOR_NAME_STRING());
        ret = -EINVAL;
        goto sensor_INIT_ERR;
    }
...
static bool sensor_fmt_capturechk(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *mf)
{
	...
	if (ret == true)
		WPRINTK("%s %dx%d is capture format\n", __FUNCTION__, mf->width, mf->height);
	return ret;
}
...
static bool sensor_fmt_videochk(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *mf)
{
	...
	if (ret == true)
		WPRINTK("%s %dx%d is video format\n", __FUNCTION__, mf->width, mf->height);
	return ret;
}
static int sensor_s_fmt(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *mf)
{
	...
	if (sensor->info_priv.fmt.code != mf->code) {
		...
		if (winseqe_set_addr != NULL) {
			...
			WPRINTK("%s v4l2_mbus_code:%d set success!\n", SENSOR_NAME_STRING(),mf->code);
		} else {
			pr_info("%s v4l2_mbus_code:%d is invalidate!\n", SENSOR_NAME_STRING(),mf->code);
		}
	}
    ...
    if ((int)winseqe_set_addr  != sensor->info_priv.winseqe_cur_addr) {
        ...
        if (sensor_fmt_capturechk(sd,mf) == true) {      /* ddl@rock-chips.com : Capture */
            if ((sensor->info_priv.flash == 1) || (sensor->info_priv.flash == 2)) {
                ...
                WPRINTK("%s flash on in capture!\n", SENSOR_NAME_STRING());
            }           
        } else {                                        /* ddl@rock-chips.com : Video */
            if ((sensor->info_priv.flash == 1) || (sensor->info_priv.flash == 2)) {
                sensor_ioctrl(icd, Sensor_Flash, Flash_Off);
                WPRINTK("%s flash off in preivew!\n", SENSOR_NAME_STRING());
            }
        }
        #endif
        ret |= sensor_write_array(client, winseqe_set_addr);
        if (ret != 0) {
            pr_err("%s set format capability failed\n", SENSOR_NAME_STRING());
            #if CONFIG_SENSOR_Flash
            if (sensor_fmt_capturechk(sd,mf) == true) {
                if ((sensor->info_priv.flash == 1) || (sensor->info_priv.flash == 2)) {
                    sensor_ioctrl(icd, Sensor_Flash, Flash_Off);
                    pr_err("%s Capture format set fail, flash off !\n", SENSOR_NAME_STRING());
                }
            }
            #endif
            goto sensor_s_fmt_end;
...
static int sensor_set_digitalzoom(struct soc_camera_device *icd, const struct v4l2_queryctrl *qctrl, int value)
{
    ...
    {
        pr_err("%s digitalzoom is maximum - %x\n", SENSOR_NAME_STRING(), digitalzoom_cur);
        return -EINVAL;
    }

    if  ((value < 0) && (digitalzoom_cur <= qctrl_info->minimum))
    {
        pr_err("%s digitalzoom is minimum - %x\n", SENSOR_NAME_STRING(), digitalzoom_cur);
        return -EINVAL;
    }
...
static long sensor_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	switch (cmd)
	{
		...
		case RK29_CAM_SUBDEV_IOREQUEST:
		{
			...
			if(j == RK_CAM_NUM){
				pr_err("%s %s RK_CAM_SUBDEV_IOREQUEST fail\n",SENSOR_NAME_STRING(),__FUNCTION__);
				ret = -EINVAL;
        	...
        	if (sensor->sensor_gpio_res) { 
                if (sensor->sensor_gpio_res->gpio_flash == INVALID_GPIO) {
                    ...
                    WPRINTK("%s flash gpio is invalidate!\n",SENSOR_NAME_STRING());
		...
		default:
		{
			pr_err("%s %s cmd(0x%x) is unknown !\n",SENSOR_NAME_STRING(),__FUNCTION__,cmd);
			break;
		}
	}
sensor_ioctl_end:
			 ...
			 const struct i2c_device_id *did)
{
	...
	//wisky-lxh@20120913,if front camera aready exist
	if (f_cam_probe_success_flag) {
		printk("%s:%d: probe failed as the front camera had been probed successfully!\n",
				__func__, __LINE__);
		return -1;
	}
...
MODULE_AUTHOR("www.wisky.com.cn");
MODULE_LICENSE("GPL");
