...
{
#if defined(WISKY_CAMERA_REG_B)
	#include WISKY_CAMERA_REG_B
#else
	#include "wisky_camera_hi253_default.h"
#endif
};
...
static int sensor_init(struct v4l2_subdev *sd, u32 val)
{
	...
	//printk("gpio_get_value(CAMERA_FRONT_PDN_PIN):%d\n",gpio_get_value(CAMERA_FRONT_PDN_PIN));    
    /* soft reset */
	//gpio_set_value(CAMERA_FRONT_PDN_PIN,1);
	if (sensor_task_lock(client,1)<0)
		goto sensor_INIT_ERR;
#if 1	
		//while(1){
...
static bool sensor_fmt_capturechk(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *mf)
{
	...
	if (ret == true)
		pr_info("%s %dx%d is capture format\n", __FUNCTION__, mf->width, mf->height);
	return ret;
}
...
static int sensor_s_fmt(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *mf)
{
	...
	if (sensor->info_priv.fmt.code != mf->code) {
		...
		if (winseqe_set_addr != NULL) {
			...
			pr_err("%s v4l2_mbus_code:%d is invalidate!\n", SENSOR_NAME_STRING(),mf->code);
		}
	}
    ...
    else if (((set_w <= 1280) && (set_h <= 720)) && (sensor_svga[0].reg != END_REG))
    {
#if 0
	winseqe_set_addr = sensor_svga;
        set_w = 1280;
			       ...
			       struct i2c_client *client)
{
	...
	//gpio_set_value(CAMERA_FRONT_PDN_PIN,1);
		//while(1){
    /* soft reset */
    //printk("gpio_get_value(CAMERA_FRONT_PDN_PIN):%d\n",gpio_get_value(CAMERA_BACK_PDN_PIN));
    ret = sensor_write(client, 0x01, 0xf9);
    if (ret != 0)
    {
