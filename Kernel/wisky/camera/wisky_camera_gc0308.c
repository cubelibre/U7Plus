...
static struct reginfo sensor_init_data[] =
{
	...
	//-------------H_V_Switch(4)---------------//
#if defined(RO_SF_HARDWARE_90)
			{0x14 , 0x10},
#elif defined(RO_SF_HARDWARE_270)
			{0x14 , 0x13},
#else
			{0x14 , 0x10},  //13//
#endif
...
static int sensor_write_array(struct i2c_client *client, struct reginfo *regarray)
{
	...
	while((regarray[i].reg != 0) || (regarray[i].val != 0))
    {
        ...
        if (err != 0)
        {
            WPRINTK("%s..write failed current i = %d\n", SENSOR_NAME_STRING(),i);
            return err;
        }
		i++;
    }
...
static int sensor_check_array(struct i2c_client *client, struct reginfo *regarray)
{
  ...
  for(i=0;i<sizeof(sensor_init_data) / 2;i++)
	 ...
	 if(ret !=0)
	 {
	  WPRINTK("read value failed\n");

	 }
	 if(regarray[i].val != value)
	 {
	  WPRINTK("%s reg[0x%x] check err,writte :0x%x  read:0x%x\n",__FUNCTION__,regarray[i].reg,regarray[i].val,value);
	 }
...
static int sensor_ioctrl(struct soc_camera_device *icd,enum rk29sensor_power_cmd cmd, int on)
{
	...
	switch (cmd)
	{
		...
		default:
		{
			WPRINTK("%s %s cmd(0x%x) is unknown!",SENSOR_NAME_STRING(),__FUNCTION__,cmd);
			break;
		}
	}
sensor_power_end:
...
static int sensor_init(struct v4l2_subdev *sd, u32 val)
{
   ...
   /* ret = sensor_write(client, 0x12, 0x80);
    if (ret != 0)
    {
        WPRINTK("%s soft reset sensor failed\n",SENSOR_NAME_STRING());
        ret = -ENODEV;
		goto sensor_INIT_ERR;
    }
    ...
    if (ret != 0)
    {
        WPRINTK("error: %s initial failed\n",SENSOR_NAME_STRING());
        goto sensor_INIT_ERR;
    }
    ...
    if (!fmt) {
        WPRINTK("error: %s initial array colour fmts is not support!!",SENSOR_NAME_STRING());
        ret = -EINVAL;
        goto sensor_INIT_ERR;
    }
...
static int sensor_s_fmt(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *mf)
{
	...
	if (sensor->info_priv.fmt.code != mf->code) {
		...
		if (winseqe_set_addr != NULL) {
			...
			WPRINTK("%s v4l2_mbus_code:%d is invalidate!\n", SENSOR_NAME_STRING(),mf->code);
		}
	}
    ...
    if ((int)winseqe_set_addr  != sensor->info_priv.winseqe_cur_addr) {
        ...
        if (ret != 0) {
            WPRINTK("%s set format capability failed\n", SENSOR_NAME_STRING());
            #if CONFIG_SENSOR_Flash
            if (sensor_fmt_capturechk(sd,mf) == true) {
                if ((sensor->info_priv.flash == 1) || (sensor->info_priv.flash == 2)) {
                    sensor_ioctrl(icd, Sensor_Flash, Flash_Off);
                    WPRINTK("%s Capture format set fail, flash off !\n", SENSOR_NAME_STRING());
                }
            }
            #endif
            goto sensor_s_fmt_end;
...
static int sensor_set_digitalzoom(struct soc_camera_device *icd, const struct v4l2_queryctrl *qctrl, int *value)
{
    ...
    {
        WPRINTK("%s digitalzoom is maximum - %x\n", SENSOR_NAME_STRING(), digitalzoom_cur);
        return -EINVAL;
    }

    if  ((*value < 0) && (digitalzoom_cur <= qctrl_info->minimum))
    {
        WPRINTK("%s digitalzoom is minimum - %x\n", SENSOR_NAME_STRING(), digitalzoom_cur);
        return -EINVAL;
    }
			       ...
			       struct i2c_client *client)
{
   ...
   /* ret = sensor_write(client, 0x12, 0x80);
    ...
    if (ret != 0) {
        pr_err("%s read chip id high byte failed\n",SENSOR_NAME_STRING());
        ret = -ENODEV;
        goto sensor_video_probe_err;
    }
    ...
    if (pid == SENSOR_ID) {
        ...
        WPRINTK("error: %s mismatched   pid = 0x%x\n", SENSOR_NAME_STRING(), pid);
        ret = -ENODEV;
        goto sensor_video_probe_err;
    }
...
WPRINTK("sensor_video_probe_err\n");
    return ret;
}
...
static long sensor_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	switch (cmd)
	{
		...
		case RK29_CAM_SUBDEV_IOREQUEST:
		{
            ...
            if (sensor->sensor_io_request != NULL) { 
	                ...
	                if (sensor->sensor_gpio_res == NULL) {
	                    pr_err("%s %s obtain gpio resource failed when RK29_CAM_SUBDEV_IOREQUEST \n",SENSOR_NAME_STRING(),__FUNCTION__);
	                    ret = -EINVAL;
	                    goto sensor_ioctl_end;
	                }
            } else {
                WPRINTK("%s %s RK29_CAM_SUBDEV_IOREQUEST fail\n",SENSOR_NAME_STRING(),__FUNCTION__);
                ret = -EINVAL;
                goto sensor_ioctl_end;
            }
		...
		default:
		{
			WPRINTK("%s %s cmd(0x%x) is unknown !\n",SENSOR_NAME_STRING(),__FUNCTION__,cmd);
			break;
		}
	}
sensor_ioctl_end:
