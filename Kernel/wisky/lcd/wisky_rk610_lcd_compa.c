/* wisky/lcd/wisky_rk610_lcd_compa.c
 *
 ...
 * Author:	leexy
 * Note:	
 *	driver for	LG/CHIMEI,
 *	00:	LG dispaly,	 	model:	LP097X02
 *	01:	reserved
 *	10:	reserved
 *	11:	CHIMEI display,	model:	BF097XN01/BF097XN02
 * ------------------------------------------------------------
 */
...
/* Timing */
static struct lcd_timing_info LCD_TIMING[]=
{
	{
		.h_pw = 280,
		.h_fp = 260,
	...
	},
	{
		...
		.v_fp = 6,
		.v_bp = 38,
		.name =" CHIMEI LCD display...BF097XNn",
	},
};

//根据LCD 屏资料填写有效区域
#define LCD_ACTIVE_WIDTH_MM          196
#define LCD_ACTIVE_HEIGHT_MM         148
...
#define S_OUT_CLK		SCALE_RATE(148500000,    53035714)
#define S_H_PW			10
#define S_H_BP			10
#define S_H_VD			1024
#define S_H_FP			60
...
//1920*1080*50
#define S1_OUT_CLK		SCALE_RATE(148500000,    44085937)
#define S1_H_PW			10
#define S1_H_BP			10
#define S1_H_VD			1024
#define S1_H_FP			56
...
//1280*720*60
#define S2_OUT_CLK		SCALE_RATE(74250000,52593750)
#define S2_H_PW			10
#define S2_H_BP			10
#define S2_H_VD			1024
#define S2_H_FP			56
...
#define S3_OUT_CLK		SCALE_RATE(74250000,52593750)
#define S3_H_PW			100
#define S3_H_BP			100
#define S3_H_VD			1024
#define S3_H_FP			96
...
//720*576*50
#define S4_OUT_CLK		SCALE_RATE(27000000,55125000) 
#define S4_H_PW			100
#define S4_H_BP			100
#define S4_H_VD			1024
#define S4_H_FP			90
...
//720*480*60
#define S5_OUT_CLK		SCALE_RATE(27000000,56531250)
#define S5_H_PW			10
#define S5_H_BP			10
#define S5_H_VD			1024
#define S5_H_FP			74
...
#endif
enum lcd_disp_model get_lcd_model(void){

int det_l = -1;
int det_h = -1;
int ret = 0;
	rk30_mux_api_set(LCD_MODEL_DET1_MUX_NAME, LCD_MODEL_DET1_MUX_MODE);
	ret = gpio_request(LCD_MODEL_DET1_PIN, "LCD MODEL DET");
	if (ret != 0){
		printk(KERN_ERR "request lcd model detect  pin fail!\n");
	}
	else{
		gpio_direction_input(LCD_MODEL_DET1_PIN);
		det_l = gpio_get_value(LCD_MODEL_DET1_PIN);
		printk("lcd detct 1 value: %d\n", det_l);
	}

	
	rk30_mux_api_set(LCD_MODEL_DET2_MUX_NAME, LCD_MODEL_DET2_MUX_MODE);
	ret = gpio_request(LCD_MODEL_DET2_PIN, "LCD MODEL DET");
	if (ret != 0){
		printk(KERN_ERR "request lcd model detect pin fail!\n");
	}
	else{
		gpio_direction_input(LCD_MODEL_DET2_PIN);
		det_h = gpio_get_value(LCD_MODEL_DET2_PIN);
		printk("lcd detct 2 value: %d\n", det_h);
		if (det_h)
			det_h = 2;
		else
			det_h = 0;
	...
	switch(det_h + det_l){
		case 0:
			...
			//break;
		case 1:
			//return ERROR_LCD_DISP;
			return DEFAULT_LCD_DISP;
			break;
		case 2:
			//return ERROR_LCD_DISP;
			return DEFAULT_LCD_DISP;
...
void set_lcd_info(struct rk29fb_screen *screen, struct rk29lcd_info *lcd_info)
{
	...
	/* Screen size */
	screen->x_res = WISKY_LCD_WIDTH;
	screen->y_res = WISKY_LCD_HEIGHT;
	...
	switch(get_lcd_model()){
	case LG_LCD_DISP:
		screen->left_margin = LCD_TIMING[0].h_bp;
		screen->right_margin = LCD_TIMING[0].h_fp;
		screen->hsync_len = LCD_TIMING[0].h_pw;
		screen->upper_margin = LCD_TIMING[0].v_bp;
		screen->lower_margin = LCD_TIMING[0].v_fp;
		screen->vsync_len = LCD_TIMING[0].v_pw;
		
		printk("LCD model choice:%s\n",LCD_TIMING[0].name);
		break;		
		
	case CHIMEI_LCD_DISP:
		screen->left_margin = LCD_TIMING[1].h_bp;
		screen->right_margin = LCD_TIMING[1].h_fp;
		screen->hsync_len = LCD_TIMING[1].h_pw;
		screen->upper_margin = LCD_TIMING[1].v_bp;
		screen->lower_margin = LCD_TIMING[1].v_fp;
		screen->vsync_len = LCD_TIMING[1].v_pw;

		printk("LCD model choice:%s\n",LCD_TIMING[1].name);
		break;
		
	case ERROR_LCD_DISP:
		printk("LCD model detect error...\n");
		break;
	...
	default:
		...
		printk("LCD model choice :default choice!\n");
