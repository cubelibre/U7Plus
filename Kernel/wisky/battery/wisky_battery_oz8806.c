...
#include "wisky_battery_oz8806.h"
...
struct struct_batt_data batt_info = {
	0, 20, 5, 391, 250, 40, 6100, 330, 0, 0, 0, 0, 0, 0, 0, 0, 4160, 0, 5, 0, 100, 100, 6100, 0, 0, 3775, 100 
};
...
static int OZ8806_PowerOnVoltToRC(void)
{
	...
	for (j=0;j<OCVNPoints;j++)
	{
		if (OCVtable[j].iVoltage==batt_info.fOCVVolt)
		{
	...
	{
		res = ((batt_info.fOCVVolt-OCVtable[j-1].iVoltage)*
...
static int OZ8806_PowerOnRecheckRC(void)
{
	int j;
	int res;
...
//		}
	...
	else
	{
		...
		//if((fRC >= 0.1*fFCC) && (fRC <= fFCC))				//a reasonable capacity in OZ8806
		{
			...
			reCheckfVolt = OZ8806_PowerOnRecheckRC();
			if ((batt_info.fRSOC > (reCheckfVolt+40)) || (batt_info.fRSOC < (reCheckfVolt-40)))
			{
				batt_info.fRSOC = reCheckfVolt;
				OZ8806_CAR_Reset(client);	
			}
		}
	}
	...
	battery_capacity = batt_info.fRSOC;
...
//		batt_info.fCellTemp = OZ8806_TemperaturemV2oC(batt_info.fCellTemp/100);
	...
	data->control = CellVoltLow;
	sRes = i2c_smbus_read_word_data(the_OZ8806->myclient, data->control);
	if(sRes > 0)
	{
		ADCValue=(u16)sRes;
		ADCValue=ADCValue >> 4;
	...
	if(sRes > 0)
	{
		...
		{		//EOC condition, this may be different during each project
			if((batt_info.fPrevCurr < batt_info.fCurr+50) && (batt_info.fCurr >= 0))
			{
				batt_info.fRSOC = 100;
...
//				yOZState = STATEIDLE;
	...
	if ((preAdjustRSOC - batt_info.fRSOC) > 3)
			OZ8806_CAR_Reset(the_OZ8806->myclient);
	if ((batt_info.fRSOC == 0 ) && (batt_info.fVolt > 3600))
	{
		batt_info.fRSOC = OZ8806_PowerOnRecheckRC();
		OZ8806_CAR_Reset(the_OZ8806->myclient);
	}
	mutex_unlock(&data->update_lock);

	if(get_dc_status() && (batt_info.fRSOC <= 0)){
		batt_info.fRSOC = 1;
	}
	battery_capacity = batt_info.fRSOC;
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		if(get_dc_status()){
			if(is_battery_charge_full()){
				val->intval = POWER_SUPPLY_STATUS_FULL;//charge full
			}else{
			...
			union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if(get_dc_status()){
			val->intval = 1;//dc insert
		}else{
			val->intval = 0;//dc remove
		}
		break;
...
static int OZ8806_resume(struct i2c_client *client)
{
	...
	if(ADCValue & CellOCVSleepMask)							//Sleep OCV detect
	{
		...
		printk("oz8806 resume sleep ocv batt_info.fOCVVolt is %d\n",batt_info.fOCVVolt);
	...
	else													//no Sleep OCV detection, use current CAR
	{
		...
		ADCValue = i2c_smbus_read_word_data(client, data->control);
			...
			batt_info.fRC = (short)(ADCValue)*batt_info.dbCARLSB;
			batt_info.fRC = batt_info.fRC / batt_info.fRsense;
			printk("oz8806 resume no sleep ocv batt_info.fRC is %d\n",batt_info.fRC);
			batt_info.fRSOC = (batt_info.fRC - batt_info.fReserved) * 100;
			batt_info.fRSOC = batt_info.fRSOC / (batt_info.fFCC - batt_info.fReserved);
			if(batt_info.fRSOC >= 100)		batt_info.fRSOC = 100;
			if(batt_info.fRSOC <= 0)		batt_info.fRSOC = 0;
