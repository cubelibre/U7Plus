...
#define airplane_mode MOBILE_WDISABLE_PIN//ljb@wisky,RK29_PIN6_PC1
...
int modem_poweron_off(int on_off)
{
	...
	if(on_off){
		...
		//wisky[]power off modem when system shutdown
		gpio_set_value(pdata->modem_power_en, !MOBILE_PWR_ON_VALUE);
	}
...
//wisky[s]
#if 0
	result = gpio_request(pdata->modem_power_en, "mu509_power_en");
 	if (result) {
...
//wisky[e]
	...
	if(result)
	{
		printk("misc_register err\n");
               ...
               printk("misc_register successful\n");
	}
	return result;
err0:
...
{
	rk30_mux_api_set(GPIO1C1_UART0RTSN_SDMMC1WRITEPRT_NAME, GPIO1H_UART0_RTS_N);
