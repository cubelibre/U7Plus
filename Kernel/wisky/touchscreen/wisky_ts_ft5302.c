...
#include "wisky_ts_ft5302.h"

static struct i2c_client *this_client;

#define CONFIG_FT5X0X_MULTITOUCH 1
#define FT5302_I2C_SPEED 	(300*1000)
...
static int ft5x0x_read_reg(u8 addr, u8 *pdata)
{
	...
	msgs[0].scl_rate = FT5302_I2C_SPEED,
	msgs[0].flags = 0;
	msgs[0].len = 1;
	msgs[0].buf = buf;
	msgs[1].addr = this_client->addr;
	msgs[1].scl_rate = FT5302_I2C_SPEED,
...
static void ft5x0x_report_value(void)
{
	...
	for (i  = 0; i < event->touch_point; i++){
	    ...
	    // LCD view area
	    {
#if 1//if ft5302 output data max: 800x600
			x = 1024*event->au16_x[i]/800;
			y = 768*event->au16_y[i]/600;
#endif
	...
	if (event->touch_point == 0 && PEN_DOWN == data->pendown) {
		//pr_info("%s[%d]--event->touch_point == 0\n", __FUNCTION__, __LINE__);
		//ft5x0x_ts_release();
...
{
	struct ft5302_platform_data *pdata = client->dev.platform_data;
	...
	err = request_irq(ft5x0x_ts->irq, ft5x0x_ts_interrupt, IRQF_TRIGGER_FALLING, "ft5302_ts", ft5x0x_ts);
	if (err < 0) {
	...
	memcpy(ft5x0x_ts->phys, "input/ts", 8);
	input_dev->name = "ft5302 Touchscreen";
	input_dev->phys = ft5x0x_ts->phys;
	input_dev->id.bustype = BUS_I2C;
#if defined(CONFIG_FT5X0X_MULTITOUCH)
	...
	input_set_abs_params(ts->input_dev,ABS_Y,0,768,0,0);
#endif
