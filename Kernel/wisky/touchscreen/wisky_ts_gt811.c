...
static s8 gtp_wakeup_sleep(struct goodix_ts_data * ts)
{
    ...
    while(retry++ < 5)
    {
        ...
        //gtp_reset_guitar(20);
		gpio_set_value(GTP_RST_PORT, GPIO_LOW);
		msleep(20);
		gpio_set_value(GTP_RST_PORT, GPIO_HIGH);
		msleep(50);
   ...
   // msleep(2);
   //gpio_direction_output(GTP_INT_PORT,1);// GTP_GPIO_AS_INPUT(GTP_INT_PORT);
   // msleep(2);
   // GTP_GPIO_OUTPUT(GTP_INT_PORT, 0);
   // msleep(2);
    //gpio_direction_output(GTP_INT_PORT,1);//GTP_GPIO_AS_INT(GTP_INT_PORT);
    //msleep(50);
	
	    gpio_request(GTP_INT_PORT, NULL);
        ...
        gpio_pull_updown(GTP_INT_PORT, 0);
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
   ...
   // s32 ret = 0;
	...
	input_set_abs_params(ts->input_dev,ABS_X,0,GTP_MAX_HEIGHT,0,0);
	input_set_abs_params(ts->input_dev,ABS_Y,0,GTP_MAX_WIDTH,0,0);
#endif    
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	int err=0;
    struct goodix_ts_data *ts;
	struct gt811_platform_data  *pdata = client->dev.platform_data;
