...
static int update_i2c_read_bytes(struct i2c_client *client, uint8_t *buf, int len)
{
...
static int update_i2c_write_bytes(struct i2c_client *client,uint8_t *data,int len)
{
    struct i2c_msg msg;
    int ret=-1;
...
return：
	...
	执行结果码，0表示正常执行
*******************************************************/
static int update_i2c_end_cmd(struct goodix_ts_data *ts)
{
    int ret;
    u8 end_cmd_data[2]={0x80, 0x00};    

    ret=update_i2c_write_bytes(ts->client,end_cmd_data,2);
    return ret;//*/
}
...
static void update_guitar_reset(s32 ms)
{
...
static u8 get_ic_msg(struct goodix_ts_data *ts, u16 addr, u8* msg, s32 len)
{
    ...
    for (i = 0; i < 5; i++)
    {
        if (update_i2c_read_bytes(ts->client, msg, ADDR_LENGTH + len) > 0)
        {
            break;
        }
    }
    update_i2c_end_cmd(ts);
...
static u8 clear_mix_flag(struct goodix_ts_data *ts)
{
    ...
    for (i = 0; i < 5; i++)
    {
        if (update_i2c_write_bytes(ts->client, buf, 3) > 0)
        {
            break;
        }
    }
...
static u8 get_ic_fw_msg(struct goodix_ts_data *ts)
{
    ...
    buf[1] = 0x22;
    buf[2] = 0x18;
    ret =  update_i2c_write_bytes(ts->client, buf, 3);
    if (ret <= 0)
    {
        return fail;
    }
...
static int guitar_update_mode( struct goodix_ts_data *ts )
{
    ...
    //step 2
    update_guitar_reset(100);

    for(retry=0;retry < 80; retry++)
    {
        //step 3
        ret =update_i2c_write_bytes(ts->client, inbuf, 0);    //Test I2C connection.
        if (ret > 0)
        {
            DEBUG_UPDATE("<Set update mode>I2C is OK!\n");
            //step 4
            msleep(100);
            ret =update_i2c_read_bytes(ts->client, inbuf, 3);
            if (ret > 0)
            {
...
static u8 guitar_nvram_store( struct goodix_ts_data *ts )
{
    ...
    for ( i = 0 ; i < 300 ; i++ )
    {
        ret = update_i2c_write_bytes( ts->client, inbuf, 3 );
        if ( ret > 0 )
            return success;
    }
...
static u8 guitar_update_nvram(struct goodix_ts_data *ts, st_fw_head* fw_head, u8 *nvram)
{
    ...
    while(length < fw_head->lenth)
    {
        ...
        ret =  update_i2c_write_bytes(ts->client, w_buf, ADDR_LENGTH + write_bytes);
        if (ret <= 0)
        {
            if (retry++ > 10)
            {
...
/*            r_buf[0] = 0x14;
            r_buf[1] = 0x00;
            r_buf[2] = 0x80;
            update_i2c_write_bytes(ts->client, r_buf, 3);
            r_buf[0] = 0x14;
            r_buf[1] = 0x00;
            update_i2c_read_bytes(ts->client, r_buf, 3);
            ...
            for (i = 0; i < 10; i++)
            {
                ret = update_i2c_read_bytes(ts->client, r_buf, ADDR_LENGTH + write_bytes);
                if (ret <= 0)
                {
                    continue;
                }
  ...
  /*      if (!(status & 0x01))
        {
            ...
            ret = update_i2c_write_bytes(ts->client, buf, 5);
            if (ret <= 0)
            {
                continue;
            }
...
static int guitar_update_proc(void *v_ts)
{
    ...
    while(retry++ < 5)
    {
        ...
        for ( i = 0; i < 10; i++)
        {
            ret = update_i2c_read_bytes(ts->client, ic_nvram, ADDR_LENGTH + fw_head.lenth);
            if (ret <= 0)
            {
                continue;
            }
