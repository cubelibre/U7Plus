...
{
	if(SPEAKER_CTRL_PIN != NULL_GPIO){
		if(on){
			gpio_set_value(SPEAKER_CTRL_PIN, SPEAKER_EN_VALUE);
		}else{
			gpio_set_value(SPEAKER_CTRL_PIN, !SPEAKER_EN_VALUE);
		}
	}else{/*Speaker mute control by codec RT5625 GPIO2 */
		if(on){
			snd_soc_write(rt5625_codec, RT5625_GPIO_SHARING, 0x01);
			snd_soc_update_bits(rt5625_codec, RT5625_GPIO_CONFIG, 1<<2, 0<<2);
			snd_soc_write(rt5625_codec, RT5625_GPIO_OUT_CTRL, 0x04);
		}else{
			snd_soc_write(rt5625_codec, RT5625_GPIO_SHARING, 0x01);
			snd_soc_update_bits(rt5625_codec, RT5625_GPIO_CONFIG, 1<<2, 0<<2);			
			snd_soc_write(rt5625_codec, RT5625_GPIO_OUT_CTRL, 0x0);
		}
	}
...
static int rt5625_hp_event(struct snd_soc_dapm_widget *w, struct snd_kcontrol *k, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMD:
		WPRINTK("aftet virtual hp power down!\n");
	...
	case SND_SOC_DAPM_POST_PMU:
		WPRINTK("after virtual hp power up!\n");
