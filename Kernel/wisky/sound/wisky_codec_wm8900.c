/* wisky/sound/wisky_codec_wm8900.c
 *
 ...
 * LOG
 * ------------------------------------------------------------
 * V001:20110303 cd huang
 *	1.Modify for wisky MID project base on wm8900.c in rk29xx paltform
 * V002:20110410 cd huang
 *	1.Reduce noise by wm8900_set_bias_level(off) when system power on
 */
...
#define SPK_CON 		SPEAKER_CTRL_PIN//RK29_PIN6_PB6
...
//CLK , PATH, VOL,POW.
	...
	//for SPK out
#if defined(WISKY_BOARD_M808_V10)
	snd_soc_write(codec, WM8900_REG_LOUT1CTL, 0x13f);
	snd_soc_write(codec, WM8900_REG_ROUT1CTL, 0x13f);
	snd_soc_write(codec, WM8900_REG_LDAC_DV, 0x1FF);
	snd_soc_write(codec, WM8900_REG_RDAC_DV, 0x1FF);
#else
	snd_soc_write(codec, WM8900_REG_LOUT1CTL, 0x135);//0x139
	snd_soc_write(codec, WM8900_REG_ROUT1CTL, 0x135);//0x139
	snd_soc_write(codec, WM8900_REG_LDAC_DV, 0x1F7);
	snd_soc_write(codec, WM8900_REG_RDAC_DV, 0x1F7);
#endif

	//for recorder
	snd_soc_write(codec, WM8900_REG_POWER1, 0x11D);//0x210D
	snd_soc_write(codec, WM8900_REG_POWER2, 0xC1BF);//0xC1AF 
#if defined(WISKY_BOARD_M808_V10)
	snd_soc_write(codec, WM8900_REG_LADC_DV, 0x01FC);
	snd_soc_write(codec, WM8900_REG_RADC_DV, 0x01FC);
#else
	snd_soc_write(codec, WM8900_REG_LADC_DV, 0x01FF);//0x01C0 
	snd_soc_write(codec, WM8900_REG_RADC_DV, 0x01FF);//0x01C0
#endif
	...
	snd_soc_write(codec, WM8900_REG_ADCPATH, 0x0066);//0x0055
...
#if defined(WISKY_BOARD_M809_V10)
	//for headphone output volume
	snd_soc_write(codec, WM8900_REG_LOUT2CTL, 0x12B);
	snd_soc_write(codec, WM8900_REG_ROUT2CTL, 0x12B);
#else
	//for headphone output volume
