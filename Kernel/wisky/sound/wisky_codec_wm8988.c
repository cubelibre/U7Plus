/* wisky/sound/wisky_codec_wm8988.c
 *
 ...
 * LOG
 * ------------------------------------------------------------
 * V001:20111228 cd huang
 *	1.Modify for wisky MID project base on wm8988.c in rk29xx paltform
 */
...
{
	if(on){
		gpio_set_value(SPEAKER_CTRL_PIN, SPEAKER_EN_VALUE);
	}else{
		gpio_set_value(SPEAKER_CTRL_PIN, !SPEAKER_EN_VALUE);
	}
	return;
...
#if defined(WISKY_BOARD_M801H_TV20) || defined(WISKY_BOARD_M801H_V10)
//针对个别硬件需要调低放音声音
	//LOUT1V & ROUT1V use 0x176 to avoice playback noise
	snd_soc_write(codec, WM8988_LOUT1V, 0x0170);//0x017f
	snd_soc_write(codec, WM8988_ROUT1V, 0x0170);//0x017f
	snd_soc_write(codec, WM8988_LDAC, 0x1ff);
	snd_soc_write(codec, WM8988_RDAC, 0x1ff);
#elif defined(WISKY_BOARD_M800C_TV20)	
	//LOUT1V & ROUT1V use 0x176 to avoice playback noise
	snd_soc_write(codec, WM8988_LOUT1V, 0x017a);//0x017f
	snd_soc_write(codec, WM8988_ROUT1V, 0x017a);//0x017f
	snd_soc_write(codec, WM8988_LDAC, 0x1ff);
	snd_soc_write(codec, WM8988_RDAC, 0x1ff);
#elif defined(WISKY_OEM_HCL_701R)	
	//LOUT1V & ROUT1V use 0x176 to avoice playback noise
	snd_soc_write(codec, WM8988_LOUT1V, 0x017a);//0x017f
	snd_soc_write(codec, WM8988_ROUT1V, 0x017a);//0x017f
	snd_soc_write(codec, WM8988_LDAC, 0x1ff);
...
#else
	//LOUT1V & ROUT1V use 0x176 to avoice playback noise
	snd_soc_write(codec, WM8988_LOUT1V, 0x0176);//0x017f
	snd_soc_write(codec, WM8988_ROUT1V, 0x0176);//0x017f
	snd_soc_write(codec, WM8988_LDAC, 0x1ff);
	snd_soc_write(codec, WM8988_RDAC, 0x1ff);
#endif

//	snd_soc_write(codec, WM8988_ADCTL2, 0x0184);
	snd_soc_write(codec, WM8988_ADCIN,  0x0100);

//针对个别硬件需要调低录音声音
#if defined(WISKY_BOARD_M800C)||defined(WISKY_BOARD_M828_V10)
	snd_soc_write(codec, WM8988_LADCIN, 0x00c0);//Microphone Gain boost 0 DB
	snd_soc_write(codec, WM8988_RADCIN, 0x00c0);//Microphone Gain boost 0 DB
	snd_soc_write(codec, WM8988_RADC, 0x1CD);//0xC3-->0 db
	snd_soc_write(codec, WM8988_LADC, 0x1CD);
	snd_soc_write(codec, WM8988_LINVOL,  0x0139);
	snd_soc_write(codec, WM8988_RINVOL,  0x0139);
#elif defined(WISKY_BOARD_M800C_TV20)||defined(WISKY_BOARD_M801H_TV20)||defined(WISKY_BOARD_M828_TV10) \
	|| defined(WISKY_BOARD_M801H_V10) || defined(WISKY_BOARD_M828_TV20)|| defined(WISKY_BOARD_M828_PMU_TV10)	
	snd_soc_write(codec, WM8988_LADCIN, 0x00c0);//Microphone Gain boost 0 DB
	snd_soc_write(codec, WM8988_RADCIN, 0x00c0);//Microphone Gain boost 0 DB
	snd_soc_write(codec, WM8988_RADC, 0x1Da);//0xC3-->0 db
	snd_soc_write(codec, WM8988_LADC, 0x1Da);
	snd_soc_write(codec, WM8988_LINVOL,  0x0139);
	snd_soc_write(codec, WM8988_RINVOL,  0x0139);
#elif defined(WISKY_BOARD_M806C)	
	snd_soc_write(codec, WM8988_LADCIN, 0x00c0);//Microphone Gain boost 0 DB
	snd_soc_write(codec, WM8988_RADCIN, 0x00c0);//Microphone Gain boost 0 DB
	snd_soc_write(codec, WM8988_RADC, 0x1b0);//0xC3-->0 db
	snd_soc_write(codec, WM8988_LADC, 0x1b0);
	snd_soc_write(codec, WM8988_LINVOL,  0x0139);
	snd_soc_write(codec, WM8988_RINVOL,  0x0139);
#elif defined(WISKY_BOARD_PC697_TV20)
	snd_soc_write(codec, WM8988_LADCIN, 0x00f0);
	snd_soc_write(codec, WM8988_RADCIN, 0x00f0);
	snd_soc_write(codec, WM8988_RADC, 0x1CF);//0xC3-->0 db
	snd_soc_write(codec, WM8988_LADC, 0x1CF);
	snd_soc_write(codec, WM8988_LINVOL,  0x0139);
	snd_soc_write(codec, WM8988_RINVOL,  0x0139);
#elif	defined(WISKY_BOARD_M818_V40)
	snd_soc_write(codec, WM8988_LADCIN, 0x00c0);//0x00f0
	snd_soc_write(codec, WM8988_RADCIN, 0x00c0);//0x00f0
	snd_soc_write(codec, WM8988_RADC, 0x1C9);//0xC3-->0 db
	snd_soc_write(codec, WM8988_LADC, 0x1C9);
	snd_soc_write(codec, WM8988_LINVOL,  0x0139);
	snd_soc_write(codec, WM8988_RINVOL,  0x0139);
#else
	snd_soc_write(codec, WM8988_LADCIN, 0x00d0);
	snd_soc_write(codec, WM8988_RADCIN, 0x00d0);
	snd_soc_write(codec, WM8988_RADC, 0x1C9);//0xC3-->0 db
	snd_soc_write(codec, WM8988_LADC, 0x1C9);
	snd_soc_write(codec, WM8988_LINVOL,  0x0139);//max 0x13f
	snd_soc_write(codec, WM8988_RINVOL,  0x0139);
#endif
	
//	snd_soc_write(codec, WM8988_SRATE, 0x100);  ///SET MCLK/8
	//snd_soc_write(codec, WM8988_PWR1, (WM_VMID50K|WM_VREF|WM_AINL|WM_AINR|WM_ADCL|WM_ADCR));
	//snd_soc_write(codec, WM8988_PWR1, 0xfc);
