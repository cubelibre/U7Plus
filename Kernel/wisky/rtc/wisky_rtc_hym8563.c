...
static int __devinit hym8563_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//wisky-lxh@20120728,防止申请中断失败后直接退出，到ALARM相关操作出现null，系统崩溃	
	if(gpio_request(client->irq, "rtc gpio"))
	{
		printk(&client->dev, "gpio request fail\n");
...
{	
	//wisky-lxh@20120808,don't disable clkout when shutdown
#if 0
    u8 regs[2];	
    int ret; 	
    //disable clkout	
