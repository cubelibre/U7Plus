...
#define INVALID_ADVALUE 				-1//10 //wisky-lxh@20120913,update key adc value is 0
	...
	if(state) {
		if(bdata->long_press_count != 0) {
			if(bdata->long_press_count % (LONG_PRESS_COUNT+ONE_SEC_COUNT) == 0){
				WPRINTK("%skey[%s]: report ev[%d] state[0]\n", 
				...
				WPRINTK("%skey[%s]: report ev[%d] state[1]\n", 
					(button->gpio == INVALID_GPIO)?"ad":"io", button->desc, button->code_long_press);
		...
		if(bdata->long_press_count <= LONG_PRESS_COUNT) {
			...
			WPRINTK("%skey[%s]: report ev[%d] state[1], report ev[%d] state[0]\n", 
...
static void callback(struct adc_client *client, void *client_param, int result)
{
	...
	{
		ddata->result = result;
		WPRINTK("%s[%d]----------adc key value: %d\n", __FUNCTION__, __LINE__, result);
	}
	for (i = 0; i < ddata->nbuttons; i++) {
		struct rk29_button_data *bdata = &ddata->data[i];
		struct rk29_keys_button *button = bdata->button;
		//wisky-lxh@20120913, redefine null key value as 'KEY_INAVLID'
		if(button->adc_value == KEY_INVALID)
			continue;
		...
		if(result < button->adc_value + DRIFT_ADVALUE &&
			...
			button->adc_state = 0;
			bdata->adc_key_count = 0;
		}
		if(bdata->state != button->adc_state && bdata->adc_key_count >= ADC_KEY_COUNT)
			mod_timer(&bdata->timer,
				jiffies + msecs_to_jiffies(DEFAULT_DEBOUNCE_INTERVAL));
	}
	return;
