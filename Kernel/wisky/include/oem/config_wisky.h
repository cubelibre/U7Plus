...
#if defined(WISKY_OEM_WISKY)

#ifndef _CONFIG_WISKY_H_
#define _CONFIG_WISKY_H_


#define MAX_CONFIG_BUFFLEN     64//最长的BUFFER长度

//A 无线和网络设置
#define ANDROID_CONFIG_A1     "0"//A1）飞行模式 0-关闭 1-开启（默认0）
#define ANDROID_CONFIG_A2     "0"//A2）WIFI 0-关闭 1-开启（默认0）
#define ANDROID_CONFIG_A3     "0"//A3）蓝牙 0-关闭 1-开启（默认0）
#define ANDROID_CONFIG_A4     "0"//A4) 以太网0-关闭,1-开启(默认0)
...
//C 声音
#define ANDROID_CONFIG_C1      "0"//C1）静音模式 0-关闭 1-开启（默认0）
#define ANDROID_CONFIG_C2      "3"//C2）振动 0-始终 1-一律不 2-仅在静音模式下来电振动 4-仅在非静音模式下来电振动（默认3）
#define ANDROID_CONFIG_C3      "70"//C3）音量 -铃声音量  0~100的百分比表示 （默认 70）
#define ANDROID_CONFIG_C4      "70"//C4）音量 -媒体音量  0~100的百分比表示 （默认 70）
#define ANDROID_CONFIG_C5      "70"//C5）音量 ---闹钟音量  0~100的百分比表示 （默认 70）
...
#define ANDROID_CONFIG_CA      "0"//CA）选择操作音 0-关闭 1-开启（默认0）
#define ANDROID_CONFIG_CB      "0"//CB）屏幕锁定提示音 0-关闭 1-开启（默认0）
...
//E 位置和安全
#define ANDROID_CONFIG_E1      "0"//E1）使用无限网络 0-关闭 1 -开启（默认0）
#define ANDROID_CONFIG_E2      "1"//E2）使用GPS卫星 0-关闭 1 -开启（默认1）
#define ANDROID_CONFIG_E5      "1"//E5）密码可见 0-关闭 1-开启（默认1）
#define ANDROID_CONFIG_E7      "0"//E7）使用安全凭证功能 0-关闭1-开启 （默认0）

//F 应用程序
#define ANDROID_CONFIG_F1      "0"//F1）未知来源 0-关闭 1-开启（默认0）
#define ANDROID_CONFIG_F6      "0"//F6）USB调试 0-关闭 1-开启 （默认1）
#define ANDROID_CONFIG_F7      "0"//F7）保持唤醒状态 0--关闭 1-开启 （默认0）
...
//L 辅助功能
#define ANDROID_CONFIG_L1     "0"//L1）辅助功能 0-关闭 1-开启（默认设置0）
#define ANDROID_CONFIG_L2     "0"//L2）按"电源"按钮结束通话 0-关闭 1-开启（默认设置0）


//M 日期和时间设置
#define ANDROID_CONFIG_M1     "1"//M1）自动   0-关闭 1-开启（默认设置1）
#define ANDROID_CONFIG_M5     "0"//M5）使用24小时格式   0-关闭 1-开启（默认设置0）
...
//Z 其他配置
#define ANDROID_CONFIG_Z1     "0"//Z1）是否支持后台定时任务查杀 0-关闭 1-开启 （默认0）
...
#else
#define ANDROID_CONFIG_Z7     "0"//Z7）GPS在空闲时是否提示关闭对话框 0--不提示 1-- 提示 （默认0）
#endif
...
#define ANDROID_CONFIG_ZA     "Wisky"//ZA）磁盘的名称  （客户需要在我的电脑中显示的磁盘名称字符串信息）
#define ANDROID_CONFIG_ZB     "Wisky"//ZB）USB信息 （客户需要在USB驱动中显示信息）
#define ANDROID_CONFIG_ZC     "wisky"//ZC）客户名称  （字符串信息）
#define ANDROID_CONFIG_ZD     WISKY_HW_VERSION //ZD）客户的产品型号 （字符串信息）
#define ANDROID_CONFIG_ZE     "V1.0"//ZE）客户的版本号 （字符串信息）
...
#define ANDROID_CONFIG_ZI	"0"//ZI)	休眠唤醒后是否显示竖屏解锁界面 0 -- 不显示，原来是什么状态就是什么状态 1-- 强制转换成竖屏显示状态来解锁（默认0）
#define ANDROID_CONFIG_ZJ	"0"//ZJ)	光传感器亮度自动调节默认开关状态0---关闭，1---打开（默认0）
#define ANDROID_CONFIG_ZK	"0"//ZK)	硬件板使用ADC 按键，0---不使用ADC 按键，1---使用ADC按键（默认0）
#define ANDROID_CONFIG_ZL     "Wisky"//ZL）ADB名称显示信息 （客户需要在PC设备管理器中看到的ADB名称）
...
#endif
#define ANDROID_CONFIG_ZO	"0"//ZO) 是否使用动态壁纸0---不使用，1---使用
...
#else
#define ANDROID_CONFIG_ZQ	"0" //ZQ)硬件上是否使用WiFi模块0--不使用，1--使用
#endif
#if WISKY_ENABLE_BLUETOOTH
#define ANDROID_CONFIG_ZR	"1" //ZR)硬件上是否使用Bluetooth模块0--不使用，1--使用
#else
#define ANDROID_CONFIG_ZR	"0" //ZR)硬件上是否使用Bluetooth模块0--不使用，1--使用
#endif
#if WISKY_ENABLE_HDMI
#define ANDROID_CONFIG_ZS	"1" //ZS)硬件上是否使用HDMI模块0--不使用，1--使用
#else
#define ANDROID_CONFIG_ZS	"0" //ZS)硬件上是否使用HDMI模块0--不使用，1--使用
#endif
#if WISKY_ENABLE_GPS
#define ANDROID_CONFIG_ZT	"1" //ZT)硬件上是否使用GPS模块0--不使用，1--使用
#else
#define ANDROID_CONFIG_ZT	"0" //ZT)硬件上是否使用GPS模块0--不使用，1--使用
#endif
...
#else
#define ANDROID_CONFIG_ZV	"0" //ZV)硬件上是否使用Camera模块0--不使用，1--使用
#endif
#if WISKY_ENABLE_VIBRATOR
#define ANDROID_CONFIG_ZW	"1" //ZW)硬件上是否使用Vibrator振动器模块0--不使用，1--使用
#else
#define ANDROID_CONFIG_ZW	"0" //ZW)硬件上是否使用Vibrator振动器模块0--不使用，1--使用
#endif

#define ANDROID_CONFIG_ZX	"0" //ZX)硬件上是否使用LightSensor光传感器模块0--不使用，1--使用

#if WISKY_ENABLE_GSENSOR
#define ANDROID_CONFIG_ZY	"1" //ZY)硬件上是否使用G-Sensor模块0--不使用，1--使用
#else
#define ANDROID_CONFIG_ZY	"0" //ZY)硬件上是否使用G-Sensor模块0--不使用，1--使用
#endif
#define ANDROID_CONFIG_ZZ	"0" //ZZ)是否使用3G通话功能，0--不使用，1--使用
#define ANDROID_CONFIG_Y1	"0" //Y1)是否开启设置菜单中的静音设置0--不开启 1--开启
#define ANDROID_CONFIG_Y2	"1" //Y2)是否开启设置菜单中的以太网设置项0--不开启 1--开启
#define ANDROID_CONFIG_Y3	"1" //Y3)默认输入法设定0-android键盘 1-wisky输入法 2-谷歌拼音输入法
#define ANDROID_CONFIG_Y4	"0" //Y4)是否屏蔽关于设备中的“电量使用情况”0--不屏蔽 1--屏蔽 （默认0）
#define ANDROID_CONFIG_Y5	"1" //Y5)是否屏蔽声音设置中的“紧急提示音” 0--不屏蔽 1--屏蔽 （默认0）
#define ANDROID_CONFIG_Y6	"http://www.wisky.hk/" //Y6)浏览器的默认主页  （字符串信息）（默认空）
#define ANDROID_CONFIG_Y7	"0" //Y7)是否在通知栏上添加时间标识    0--不添加 1--添加 （默认0）
...
#define ANDROID_CONFIG_YA	"0" //YA)禁止系统进入深度睡眠模式 0--开启，1--禁止 （默认0）
#define ANDROID_CONFIG_YB	"0" //YB)触感是否使用连续振动0---按下时振动一次，1---按下时连续振动
#define ANDROID_CONFIG_YC	"0" //YC)是否使用连接指定WiFi网络功能0---不使用，由用户手动连接1---自动连接用户指定WiFi
#define ANDROID_CONFIG_YD	"0" //YD)是否使用状态栏音量控制按钮0---默认使用，1---不使用
#define ANDROID_CONFIG_YE	"0" //YE)WiFi休眠策略:0---屏幕关闭时休眠,1---充电时永不休眠,2---永不休眠
#define ANDROID_CONFIG_YF	"0"//YF)默认是否使用状态栏返回键0---使用，1---不使用(默认0)
#define ANDROID_CONFIG_YG	"0"//YG)是否显示本机硬件配置信息 0-不显示 1-显示2-只显示Flash内存总容量(默认0)
...
#define ANDROID_CONFIG_YQ	"0"//YQ)GoogleMarket应用，0-显示应用无设置，1-不显示应用有设置,2-显示应用有设置
...
#define ANDROID_CONFIG_YT	"1"/*YT)机器通过USB连接电脑是否直接挂载存储设备到电脑
								...
								2、弹出操作界面且自动连接
								3、自动连接但不弹出操作界面*/
#define ANDROID_CONFIG_YU	"0"//YU)桌面切换工作区时是否显示区域框，0---不显示，1---显示(默认0)
...
#define ANDROID_CONFIG_X6	"0"//X6)状态栏下拉菜单中是否含任务查杀:0--不含(默认)，1--含有
#define ANDROID_CONFIG_X7	"1"//X7)是否使用触摸按键灯:0---不使用，1---使用(默认)
#define ANDROID_CONFIG_X8	"0"//X8)是否屏蔽MENU键解锁功能:0---不屏蔽,1--屏蔽MENU解锁功能 
#define ANDROID_CONFIG_X9	"0000000000"//X9)Wisky自定义桌面控件是否显示在桌面添加菜单中，0--否，1--是
...
#define ANDROID_CONFIG_XR	"0"//XR//3G上网卡和USB以太网支持列表:0--隐藏(默认),1--显示
