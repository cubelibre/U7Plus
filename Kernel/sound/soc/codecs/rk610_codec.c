...
#define RK610_SPK_CTRL_PIN  RK30_PIN2_PA1
#define RK610_MUTE_PIN		RK30_PIN1_PC2		//headphone mute pin
#else
...
static int rk610_codec_mute(struct snd_soc_dai *dai, int mute)
{
    ...
    rk610_codec_write(codec,ACCELCODEC_R04, ASC_INT_MUTE_L|ASC_INT_MUTE_R|ASC_SIDETONE_L_OFF|ASC_SIDETONE_R_OFF);  //soft mute
		...
		//gpio_direction_output(RK30_PIN1_PC2, 1);
...
static int OBREEYSERIAL_PROC_init(void);
...
//	rk610_codec_set_bias_level(codec, SND_SOC_BIAS_STANDBY);
	...
	#if 1
		rk30_mux_api_set(GPIO1C2_CIF1DATA4_RMIITXD1_NAME, GPIO1C_GPIO1C2);
		ret = gpio_request(RK30_PIN1_PC2, "codec mute");
		if (ret != 0) {
			gpio_free(RK30_PIN1_PC2);
			printk("codec mute error\n");
			return -EIO;
		}
		gpio_direction_output(RK30_PIN1_PC2, 1);
		msleep(100);
...
static ssize_t OBREEYSERIAL_PROC_read(char *buf,char **start,off_t offset,int count,int *eof,void *data)
{
    int snlen = 20;
    char serialnum[528] = {0};
    GetSNSectorInfo(serialnum);
    memcpy(buf, &(serialnum[2]), snlen);
    return snlen;
}

static int OBREEYSERIAL_PROC_init(void)
{
    struct proc_dir_entry *OBREEYSERIAL_PROC_entry;
    OBREEYSERIAL_PROC_entry = create_proc_read_entry("driver/serialno", 0444, NULL, OBREEYSERIAL_PROC_read, NULL);
    if(OBREEYSERIAL_PROC_entry != NULL)
    {
        return 0;
    }
    printk("Can't create serial proc entry!\n");
    return -1;
}
