...
#if defined(WISKY_LOGO_CHARGE_H)
extern int __init wisky_boot_charge_main(void);


static int is_recovery_mode(void)
{
	return (strstr(boot_command_line, "(parameter)") != NULL)?1:0;
}
#endif


extern void fb_show_charge_logo(struct linux_logo *logo);
static unsigned char blank_clut224_data[WISKY_LCD_WIDTH*WISKY_LCD_HEIGHT] = {0x20};
static unsigned char blank_clut224_clut[] = {
	0x00, 0x00, 0x00
};

struct linux_logo rk_blank_clut224 __initdata = {
...
#if defined(WISKY_LOGO_CHARGE_H)
	if(!strcmp(ANDROID_CONFIG_Z6, "1")){
		printk("rk_fb_register:%d is_recovery_mode:%d\n",id,is_recovery_mode());
		
		if(is_recovery_mode() == 0 && (0 == id)){
			wisky_boot_charge_main();
		}
	}
#endif
