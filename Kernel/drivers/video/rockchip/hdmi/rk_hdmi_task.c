...
extern int earphone_hdmi_insert;
#endif

#define HDMI_MAX_TRY_TIMES	3///1
...
void hdmi_work(struct work_struct *work)
{
	...
	//printk(hdmi->dev, "[%s] done,while hdmi->state:%d, state_last:%d, trytimes:%d \n", __FUNCTION__, hdmi->state, state_last, trytimes);
	mutex_unlock(&work_mutex);
}
