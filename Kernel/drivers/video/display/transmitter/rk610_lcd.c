...
enable) |LVDS_PLL_PWR_PIN(enable) \
            ...
            |LVDS_OUTPUT_FORMAT(0x11) | LVDS_BIASE_PWR(1);
...
extern void rk30_backlight_off(void);
void rk610_lcd_suspend(void)
{
	if((lcd_shutdown_flag++) > 1)
		return ;
	rk30_backlight_off();
	msleep(100);
...
void rk610_lcd_early_suspend(struct early_suspend *h)
{
    struct i2c_client *client = g_lcd_inf->client;
    char c;
    RK610_DBG(&client->dev,"%s \n",__FUNCTION__);
...
void rk610_lcd_early_resume(struct early_suspend *h)
{
    struct i2c_client *client = g_lcd_inf->client;
    char c;
    RK610_DBG(&client->dev,"%s \n",__FUNCTION__);
