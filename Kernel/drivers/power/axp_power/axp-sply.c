...
int axp_pwr_pressshort = 0;//0---no press, 1--short press
int axp_pwr_presslong = 0;//0---no press, 1--long press
//wisky[e]
static int blink_flag = 0;
#define DBG_AXP_PSY 0 //Lee
#if  DBG_AXP_PSY
...
static void axp_charger_update_state(struct axp_charger *charger)
{
	...
	dc_adaptor_status = charger->ext_valid;
	if(!strcmp(ANDROID_CONFIG_XM, "1")){
		if(charger->ext_valid){
			if(false == wakelock_state){
				wake_lock(&wakelock);
				wakelock_state = true;
			}
		}else{
			if(true == wakelock_state){
...
static void charge_led_blink(struct work_struct *work)
{
    struct axp_charger *charger = container_of(work, struct axp_charger, work_led_blink);
    if (charger->is_on){
         if (100 == charger->rest_cap ){
              if(0 == blink_flag){
                     gpio_set_value(CHARGE_BLINK_PIN, 1);
                   //  printk("full charge blink ...on\n");
                    blink_flag = 1;
                }
            
         }else{        
             if (blink_flag){
                     gpio_set_value(CHARGE_BLINK_PIN, 0);
                   //    printk("charge blink ...off\n");
                      blink_flag = 0;
                }
                else{
                        gpio_set_value(CHARGE_BLINK_PIN, 1);
                 //    printk("charge blink ...on\n");
                      blink_flag = 1;
                    }
    ...
    schedule_delayed_work(&charger->work_led_blink, charger->interval_led_blink);
}
#endif
...
static int axp_battery_probe(struct platform_device *pdev)
{
	...
	if(!strcmp(ANDROID_CONFIG_XM, "1")){
		wakelock_state = false;
		wake_lock_init(&wakelock, WAKE_LOCK_SUSPEND, "battery_wake");
	}
	...
	INIT_DELAYED_WORK(&charger->work, axp_charging_monitor);
        ...
        #if WISKY_BOARD_U7PLUS_TV10
	/* 创建充电指示灯队列 */
	charger->interval_led_blink = msecs_to_jiffies(1 * 1000);
	INIT_DELAYED_WORK(&charger->work_led_blink, charge_led_blink);     
        if(charger->bat_det == 1){
		schedule_delayed_work(&charger->work_led_blink, charger->interval_led_blink);
	} else {
		charger->rest_cap = 100;
	...
	if(charger->bat_det == 1){
		...
		charger->rest_cap = 100;
		battery_capacity = charger->rest_cap;
	}	
