...
struct wm831x_rtc *gWm831x_rtc;

//return 0 if success, else return -1
int wisky_rtc_read_time(struct rtc_time *tm)
{
//	struct wm831x_rtc *wm831x_rtc = dev_get_drvdata(dev);
	struct wm831x *wm831x = gWm831x_rtc->wm831x;
	u16 time1[2], time2[2];
	int ret;
	int count = 0;
	...
	if (ret < 0) {
		pr_err("Failed to read RTC control: %d\n", ret);
		return ret;
	}
	if (!(ret & WM831X_RTC_VALID)) {
		pr_err("RTC not yet configured\n");
		return -EINVAL;
	}
	...
	pr_err("Timed out reading current time\n");

	return -EIO;
}
EXPORT_SYMBOL(wisky_rtc_read_time);
//wisky[e]
...
subsys_initcall_sync(wm831x_rtc_init);
